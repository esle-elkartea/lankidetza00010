...
function escribe_formulario_user_vacio($nombrecampo,$nombreformulario,$idEmpresa){
	global $raiz;
	echo '<select name="'.$nombrecampo.'" size="3" multiple class="text_normal" >	</select>	<input type=button value="Asignar Usuario" onclick="muestraVentana(\''. $raiz.'\',\''. $nombreformulario .'\',\''.$nombrecampo.'\',\''. $idEmpresa .'\')">';	
}



function mostrar_user($idEmpresa){
	...
	<tr><form action="asigUser.php?'. $parametros_formulario . '" method="POST">';
	...
	if ($mes_anterior==0){
	  ...
	  */
		...
		$sql="select Nombre,Apellidos  from Usuarios where idEmpresa=".$idEmpresa;
     $r = mysql_query($sql);
     while ($rU = mysql_fetch_array($r,MYSQL_ASSOC))
      {
		echo '	<tr>
			    <td width=14% align=center class=altn> <select name="Usuario" size="1" class="text_normal" id="JefeObraProyecto">
                <option>[Seleccione un usuario]</option><option>';
                
    echo $rU['Nombre']." ".$rU['Apellidos']; 
         }  
	...
	<tr>
	    <td colspan="2" align="center"><input type="Submit" name="Asignar" value="Asignar ese usuario"></td>
	</tr>
	</table><br>
